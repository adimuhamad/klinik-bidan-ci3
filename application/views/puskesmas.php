<div class="content-wrapper">
  <section class="content-header">
      <h1>
        Data Puskesmas Terdekat
      </h1>
    </section>
    <section class="content">
      <?php echo $this->session->flashdata('pesan') ?>
      <?php echo $this->session->flashdata('message'); ?>
      <button class="btn btn-primary" data-toggle="modal" data-target="#exampleModal"><i class="fa fa-plus"> Tambah Data Puskesmas</i></button>

      <!-- <div class="navbar-form navbar-right">
        <?php echo form_open('puskesmas/search') ?>
    ...
    <div style="margin-top: 20px">
      <table id="dataPuskes" class="table table-bordered table-striped">
        <thead>
          <tr>
            <th>No</th>
            <th>Nama</th>
        ...
        $no = 1;
        foreach ($tb_pkm as $pkm) : ?>

        <tr>
          <td><?php echo $no++ ?></td>
          <td><?php echo $pkm->namapkm ?></td>
          <td><?php echo $pkm->alamatpkm ?></td>
          <td><?php echo $pkm->notelp ?></td>
          <td><?php echo $pkm->jadwalbuka ?></td>
          <td><?php echo $pkm->jamoperasional ?></td>

          <td><?php echo anchor('puskesmas/edit/'.$pkm->idPkm, '<center><div class="btn btn-primary"><i class="fa fa-edit"></i></div>') ?></td>

          <td onclick="javascript: return confirm('Apakah anda yakin ingin menghapus data ini?')">
            <?php echo anchor('puskesmas/hapus/'.$pkm->idPkm, '<center><div class="btn btn-danger"><i class="fa fa-trash"></i></div>') ?></td>
        </tr>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="exampleModalLabel">Form Input Data Puskesmas</h4>
      </div>
      <div class="modal-body">
        <form method="post" action="<?php echo base_url().'puskesmas/tambah_aksi' ?>">
            ...
            var address = document.getElementById('namapkm').value;
          ...
          <div class="form-group">
            ...
            <input type="text" onChange="geocodeLokasi()" name="namapkm" id="namapkm" class="form-control" placeholder="Masukan Nama Puskesmas" required>
          </div>

          <div class="form-group">
            <label>Alamat Puskesmas</label>
            <input type="text" name="alamatpkm" id="alamatpkm" class="form-control" placeholder="Masukan Alamat Puskesmas" required>
          </div>
